...
class GApiResponse {
    ...
    class Autocomplete {
        ...
        class func initWithData(_ data:[String:Any]) -> Autocomplete {
            ...
            if let pId = data["place_id"] as? String,let address = data["structured_formatting"] as? NSDictionary {
               
                object.formattedAddress = address["main_text"] as? String ?? ""
                object.placeId = pId
                object.secondaryText =  address["secondary_text"] as? String ?? ""
...
class GoogleApi : NSObject {
    ...
    func getUrl(_  usedFor:UsedFor,input:String) -> String {
        ...
        case .autocomplete:
            ...
            return String(format: "%@?input=%@&components=country:sg&key=%@",GoogleUrl.autocomplete.rawValue,updatedInput,googleApiKey)
        case .placeInformation:
    ...
    func PlaceDetailCallApi(_ api : UsedFor = .placeInformation,input:GInput,completion: @escaping GCallback) {
        let covertedInput = getConvertedInputFor(api,input:input)
        guard !covertedInput.isEmpty && !googleApiKey.isEmpty else {
            let eDesc = googleApiKey.isEmpty ? "Please add valid google api key" : "Some error occured"
            print("Google api error - \(eDesc)")
